<?php
acms_header('Редактировать сообщение', 'users');
get_check_valid();

$comments = db::get_string("SELECT `ID`,`MESSAGE`,`USER_ID` FROM `COMMENTS` WHERE `ID` = ? LIMIT 1", [intval(get('id'))]);

if ($comments['USER_ID'] != user('ID') && access('comments', null) == false) {
...
if (isset($comments['ID']) && get('action') && get('type')) {
  ...
  <a class='button-o' href='<?=$return?>'><?=lg('Отмена')?></a>
  </form>
  </div>
  <?
